...
def can_borrow(
    ...
    Determine whether a Loan can occur.
        ...
        Returns:
            True if the patron is allowed to borrow the item,
            otherwise false. False if an invalid item type is provided.
    '''
    if type_of_item == "Book":
        return can_borrow_book(
            patron_age, length_of_loan, outstanding_fees
        )
...
def can_borrow_book(patron_age, length_of_loan, outstanding_fees):
    ...
    Determine whether a book Loan can occur.
        Args:
            ...
            the fees the patron owes, before any discounts are considered.
        Returns:
            True if the patron is allowed to borrow the book,
            otherwise false.
            A patron is allowed to borrow a book for up to
            8 weeks if they have no fees owed (i.e., fees
...
def can_borrow_gardening_tool(
        patron_age, length_of_loan,
        outstanding_fees, gardening_tool_training
    ):
    '''
    Determine whether a gardening tool Loan can occur.
        ...
        Returns:
            ...
            They are not allowed to borrow a gardening tool
            if they have not completed the gardening tool
...
def can_borrow_carpentry_tool(
    ...
    Determine whether a carpentry tool Loan can occur.
        ...
        Returns:
            ...
            They are not allowed to borrow a carpentry
            tool if they have not completed the carpentry tool
            training, if they have fees owed (i.e.,
            fees to pay after discounts are considered), or if
...
def can_use_makerspace(patron_age, outstanding_fees, makerspace_training):
    ...
    Determine whether a patron can use the makerspace.
        Args:
            ...
            outstanding_fees (float): the fees the patron owes,
            before any discounts are considered.
            makerspace_training (bool): whether the patron has
            completed the makerspace training or not.
        Returns:
    ...
    if patron_type == "ERROR": # if
        result = False
    elif patron_type in ("Elderly", "Minor"): #elif
        result = False
    else: #else
        discount = calculate_discount(patron_age)
        fees_owed -= outstanding_fees * (discount / 100)

    if result is True and fees_owed > 0: #if
        result = False
    return result
...
def process_loan(patron, item, length_of_loan):
    ...
    if can_borrow(item._type, patron._age,
                length_of_loan, patron._outstanding_fees,
                patron._gardening_tool_training,
                patron._carpentry_tool_training):
        new_loan = loan.Loan(item, due_date)
